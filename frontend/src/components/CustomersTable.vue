      ...
      <div class="col-12">
        <div class="card shadow-sm">
          <div class="card-header category bg-dark text-white d-flex justify-content-between align-items-center">
            ...
            <div>
              <div class="dropdown d-inline me-2">
                <button class="btn btn-sm btn-success dropdown-toggle" type="button" id="generateReportDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                  Generate Report
                </button>
          ...
          <div class="card-body">
            ...
            <div class="row mb-3">
              <div class="col-md-9">
                <input type="text" v-model="searchQuery" class="form-control" placeholder="Search by ID, Username or Shipping Addresses">
              </div>
              <div class="col-md-3">
                ...
                <tr v-for="customer in filteredCustomers" :key="customer.id">
                  ...
                  <td>
                    <ul style="list-style-type: square; padding-left: 1rem; margin-bottom: 0;">
                      <li v-for="address in (customer.shippingAddresses ? customer.shippingAddresses.split(' | ') : [])" :key="address">
                        {{ address }}
                      </li>
                    </ul>
                  </td>
      ...
      filteredCustomers() {
        ...
        if (this.filterDOB) {
          filtered = filtered.filter((customer) => {
            const customerDOB = new Date(customer.dateOfBirth).toISOString().slice(0, 10);
            const filterDOB = new Date(this.filterDOB).toISOString().slice(0, 10);
